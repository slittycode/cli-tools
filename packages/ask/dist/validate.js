...
import { MAX_AGENT_FLAG_CHARS, MAX_PROMPT_BYTES, MAX_STDIN_BYTES, MAX_SYSTEM_CHARS, } from './config.js';
 ...
 */
export function validateAgentFlag(raw) {
    const trimmed = raw.trim();
    if (trimmed.length === 0) {
        throw new AskError(ErrorCode.INVALID_AGENT_FLAG, '--agent value cannot be empty');
    }
    if (trimmed.length > MAX_AGENT_FLAG_CHARS) {
        throw new AskError(ErrorCode.INVALID_AGENT_FLAG, `--agent value too long (max ${MAX_AGENT_FLAG_CHARS} chars)`);
    }
    if (!AGENT_FLAG_RE.test(trimmed)) {
        throw new AskError(ErrorCode.INVALID_AGENT_FLAG, `Invalid --agent format "${trimmed}". Expected <id> or <id>:<model>, e.g. "ollama:qwen2.5"`);
    }
    const colonIdx = trimmed.indexOf(':');
    const id = (colonIdx === -1 ? trimmed : trimmed.slice(0, colonIdx));
    const model = colonIdx === -1 ? undefined : trimmed.slice(colonIdx + 1);
    if (!VALID_AGENT_IDS.has(id)) {
        throw new AskError(ErrorCode.UNKNOWN_AGENT, `Unknown agent "${id}". Valid agents: ${[...VALID_AGENT_IDS].join(', ')}`);
    }
    return { id, model };
}
/**
...
export function validatePrompt(prompt) {
    ...
    if (byteLen > MAX_PROMPT_BYTES) {
        ...
        process.stderr.write(`Warning: prompt is ${Math.round(byteLen / 1024)} KB — some agents may truncate or reject it.\n`);
    }
    return prompt;
}
/**
 ...
 */
export function validateStdinSize(bytesSoFar) {
    if (bytesSoFar > MAX_STDIN_BYTES) {
        throw new AskError(ErrorCode.STDIN_TOO_LARGE, `stdin exceeds the ${MAX_STDIN_BYTES / 1_000_000} MB limit. Pipe a smaller input.`);
    }
}
/**
 ...
 */
export function validateSystemPrompt(raw) {
    const trimmed = raw.trim();
    if (trimmed.length > MAX_SYSTEM_CHARS) {
        throw new AskError(ErrorCode.SYSTEM_TOO_LONG, `--system value is too long (${trimmed.length} chars, max ${MAX_SYSTEM_CHARS})`);
    }
    return trimmed;
}
/**
...
export function validateModelName(model) {
    ...
    if (!/^[A-Za-z0-9][A-Za-z0-9._:-]*$/.test(model)) {
        throw new AskError(ErrorCode.INVALID_AGENT_FLAG, `Invalid model name "${model}". Use letters, digits, hyphens, dots, and colons only.`);
    }
    return model;
}
